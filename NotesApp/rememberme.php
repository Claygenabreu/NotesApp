...
//alternative function for the following: array_key_exists('user_id', $_SESSION)99999999

if(!isset($_SESSION['user_id']) && !empty($_COOKIE['rememberme'])){
  //f1: COOKIE: $a . "," . bin2hex($b);
  //f2: hash('sha256', $a);
  //extract $authentificator 1 & 2from the cookie
  ...
  // Look for the $authenticator1 in teh rememberme table
...
//if $authenticator2 does not match-
  ...
  echo '<div class="alert alert-danger">hash_equals returned false.</div>';
    ...
    //  store them in cookie and remeberme table
